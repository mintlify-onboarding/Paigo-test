...
This document provides a high-level overview of Paigo platform's functionalities and how we achieved them. The table below demonstrates the critical components of usage-based business operations, which are also the functionality areas of Paigo platform.
...
* Various pricing structure: pay-as-you-go, subscription tier, flat rate, or hybrid.

* Pricing add-on: usage entitlement, overage charge, free trial, discount

* B2B sales deal: private contract, commitment, prepaid credit
...
Usage Measurementand CollectionBillingAggregationInvoicingand TaxCloud Marketplace ReportingPaymentProcessing

* **Measure customer usage** Paigo's unique usage measurement engine deeply integrates with SaaS application architecture to calculate precise usage in real-time. The usage measurement engine supports infrastructure-based, agent-based, datastore-based, SQL-based and API-based usage measurement. See [Measure and Collect Usage Data at Scale](/measure-usage-and-collect-data/measure-and-collect-usage-data-at-scale) chapter for more information.

* **Collect usage data** Delivery the measurement the usage data to backend billing system with client-side transformation, high-availability and reliability. Usage data collection in Paigo is also done by usage measurement engine.

* **Persist billing data** Once the usage data is delivered at the backend, Paigo applies algorithms to process data to ensure it's uniqueness, and persist the data point in database as time-series append-only usage journal with proper indexing for billing and analytics use cases.

* **Aggregate usage** Run batch processes to aggregate raw data layer into different aggregated layer, such as hourly sum, daily maximum, or monthly average. Also group data by common use cases such as product lines, customers, or pricing plans.

* **Invoice and payment** At the end of billing cycle, scheduled processes run to calculate all the service usage by each customer and the total amount owed. Tax-compliant invoice is generated for review. Payment requests are issued upon the approval of the invoice. Paigo also takes care of failed payment and revenue recovery. See [Process Payment with Stripe](/invoice-and-process-payment/process-payment-with-stripe) chapter for more information.
...
* Variable cost as designated by usage dimensions
